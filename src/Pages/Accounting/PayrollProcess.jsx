...
const PayrollProcess = () => {
  ...
  const employees = [
    { id: 1, name: "Maulana Ahmed", role: "Head Teacher", basic: 25000, allowance: 5000, deduction: 0, status: "Pending" },
    { id: 2, name: "Hafiz Karim", role: "Teacher", basic: 18000, allowance: 2000, deduction: 500, status: "Paid" },
    { id: 3, name: "Abdul Jabbar", role: "Staff", basic: 12000, allowance: 1000, deduction: 0, status: "Pending" },
  ];
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
           ...
           <p className="text-slate-500">Disburse salaries for <span className="font-bold text-slate-800">January 2025</span></p>
        </div>
        <div className="flex gap-3">
             <button className="px-4 py-2 border border-slate-200 rounded-lg text-slate-600 font-semibold hover:bg-slate-50">View History</button>
             <button className="px-6 py-2 bg-primary text-white font-bold rounded-lg shadow-lg shadow-primary/20 hover:scale-105 transition-all">Process All</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Total Payable</p>
          ...
          <div className="bg-indigo-50 p-6 rounded-2xl border border-indigo-100 shadow-sm">
              <p className="text-xs font-bold text-indigo-400 uppercase tracking-widest mb-2">Disbursed</p>
          ...
          <div className="bg-amber-50 p-6 rounded-2xl border border-amber-100 shadow-sm">
              <p className="text-xs font-bold text-amber-500 uppercase tracking-widest mb-2">Pending</p>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
         <div className="p-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between gap-4">
             ...
             <div className="relative max-w-sm w-full">
                 ...
                 <input type="text" placeholder="Search employee..." className="w-full pl-9 pr-4 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:border-primary" />
             </div>
         </div>
             ...
             <table className="w-full text-left border-collapse">
                 <thead>
                     <tr className="bg-slate-50 text-slate-500 text-xs uppercase font-bold tracking-wider">
                         ...
                         <th className="p-4 border-b border-slate-200">Base Salary</th>
                         <th className="p-4 border-b border-slate-200">Allowances</th>
                         <th className="p-4 border-b border-slate-200 text-rose-500">Deductions</th>
                         <th className="p-4 border-b border-slate-200 text-emerald-600">Net Payable</th>
                     ...
                     {employees.map((emp) => {
                         ...
                         return (
                            <tr key={emp.id} className="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td className="p-4 text-slate-600 font-medium">৳ {emp.basic}</td>
                                <td className="p-4 text-green-600 font-medium">+ {emp.allowance}</td>
                                <td className="p-4 text-rose-500 font-medium">- {emp.deduction}</td>
                                <td className="p-4 font-black text-slate-800">৳ {netPay}</td>
