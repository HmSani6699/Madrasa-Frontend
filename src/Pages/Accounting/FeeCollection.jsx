  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-8 animate-in fade-in duration-500">
      ...
      <div>
        ...
        <p className="text-slate-500">Search for a student to collect dues.</p>
      </div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row gap-4 items-center">
         ...
         <button 
           ...
           className="px-8 py-3 bg-primary text-white font-bold rounded-xl shadow-lg shadow-primary/20 hover:scale-105 transition-transform w-full md:w-auto"
         >
           Find Student
         </button>
      </div>
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-6">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-400 mt-1">Guardian: {selectedStudent.guardian}</p>
                    </div>
                </div>
            ...
            <div className="space-y-6">
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
                    ...
                    <div className="space-y-4">
                         ...
                         <div>
                            <label className="text-xs font-bold text-slate-500 uppercase flex justify-between">
                                Discount / Waiver
                                <span className="text-[10px] text-primary cursor-pointer hover:underline">Apply Preset?</span>
                            </label>
                            <input 
                              type="number" 
                            ...
                            />
                            <p className="text-[10px] text-slate-400 mt-1">Deducting from ৳ {subTotal}</p>
                         </div>
                         ...
                         {/* Paid Amount Input */}
                         <div>
                            <label className="text-xs font-bold text-slate-500 uppercase">Paid Amount (Cash/Bank)</label>
                            <input 
                              type="number" 
                         ...
                         {grandTotal > 0 && (
                             <div className={`p-4 rounded-xl text-sm font-bold flex justify-between items-center ${paidVal < grandTotal ? 'bg-amber-50 text-amber-700' : 'bg-emerald-50 text-emerald-700'}`}>
                                 <span>{paidVal < grandTotal ? 'Remaining Due' : 'Status'}</span>
                                 <span className="text-lg">{paidVal < grandTotal ? `৳ ${due}` : 'Fully Paid'}</span>
                             </div>
                         )}
                    ...
                    <button className="w-full py-4 bg-emerald-600 text-white font-bold rounded-xl shadow-lg shadow-emerald-200 hover:bg-emerald-700 transition-all flex items-center justify-center gap-2">
                        ...
                        {paidVal < grandTotal ? 'Confirm Partial Payment' : 'Confirm Payment'}
                    </button>
