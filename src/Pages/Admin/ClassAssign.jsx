...
const ClassAssign = () => {
  ...
  const availableClasses = ["Class 5", "Class 6", "Class 7", "Hifz Class", "Kitab Class"];
  const availableSections = ["Section A", "Section B", "Morning", "Evening"];
  const availableSubjects = [
    "Arabic Grammar", "Arabic Literature", "Fiqh", "Hadith Studies", 
    "Quran Memorization", "Mathematics", "English Foundation", "Science"
  ];
  ...
  const handleAction = () => {
    if (modalType === "add") {
      ...
      setClassMappings(classMappings.map(m => m.id === selectedMapping.id ? { ...m, ...formData } : m));
    }
    ...
    <div className="space-y-10 animate-in fade-in duration-500 p-6 md:p-10">
      ...
      <div className="bg-white rounded-[3rem] border-2 border-slate-200 p-10 flex flex-col lg:flex-row justify-between items-center gap-8 shadow-sm relative overflow-hidden">
        <div className="flex items-center gap-8 relative z-10">
          ...
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-slate-800 tracking-tight text-center lg:text-left">Class Assignment Map</h1>
            <p className="text-slate-500 font-bold mt-2 text-lg text-center lg:text-left">Bridge sections and subjects to institutional classes</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {filteredMappings.map((mapping) => (
          <div key={mapping.id} className="group bg-white rounded-[3rem] border-2 border-slate-100 shadow-xl shadow-slate-100/20 hover:border-indigo-500/20 transition-all duration-300 overflow-hidden flex flex-col relative">
            {/* Action Bar */}
            ...
            <div className="p-10 space-y-8">
              <div className="flex items-center gap-5">
                ...
                <div>
                   <h2 className="text-2xl font-black text-slate-800 group-hover:text-indigo-600 transition-colors uppercase tracking-tight">{mapping.className}</h2>
                   <div className="flex items-center gap-2 mt-2">
                       <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                       <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Active Mapping</span>
                   </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                    <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest pb-2 border-b-2 border-slate-50">
                        <Hash className="w-3.5 h-3.5" /> Mapped Sections
                    </div>
                    <div className="flex flex-wrap gap-2 pt-1">
                        {mapping.sections.map(sec => (
                            <span key={sec} className="px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl text-[10px] font-black border border-indigo-100/50 shadow-sm">
                                {sec}
                            </span>
                        ))}
                        {mapping.sections.length === 0 && <span className="text-xs font-bold text-slate-300 italic">No sections mapped</span>}
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-2 pt-1">
                        {mapping.subjects.map(sub => (
                            <span key={sub} className="px-4 py-2 bg-amber-50 text-amber-600 rounded-xl text-[10px] font-black border border-amber-100/50 shadow-sm">
                                {sub}
                            </span>
                        ))}
                        {mapping.subjects.length === 0 && <span className="text-xs font-bold text-slate-300 italic">No subjects allocated</span>}
                    </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-auto px-10 py-6 bg-slate-50/50 flex items-center justify-between border-t-2 border-slate-50">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                    Total Assets: <span className="text-slate-800">{mapping.sections.length + mapping.subjects.length} Units</span>
                </p>
                <div className="flex items-center gap-2 text-indigo-500 group-hover:translate-x-1 transition-transform">
                    <span className="text-xs font-black uppercase tracking-tighter">Modify Specs</span>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md z-[100] flex items-center justify-center p-6 lg:p-10">
          <div className="bg-white rounded-[3rem] w-full max-w-4xl shadow-2xl animate-in scale-in duration-300 overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        <h2 className="text-2xl font-black text-slate-800 tracking-tight">
                            {modalType === 'add' ? 'Institutional Class Mapping' : 'Reconfigure Academic Map'}
                        </h2>
                        <p className="text-xs font-black text-slate-400 uppercase tracking-widest mt-1">Cross-Academic Unit Assignment</p>
                    </div>
                </div>
             ...
             <div className="p-10 space-y-10 overflow-y-auto">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest">Primary Identity</h3>
                        </div>
                        <div className="space-y-4">
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-400 uppercase">Target Institutional Class</label>
                                <select 
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-400 uppercase">Operational Status</label>
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest">Section Mapping</h3>
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                           {availableSections.map(sec => (
                               ...
                               >
                                  {sec}
                                  {formData.sections.includes(sec) ? <CheckCircle2 className="w-3 h-3" /> : <Plus className="w-3 h-3" />}
                               </button>
                           ))}
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest">Subject Allocation</h3>
                        </div>
                           ...
                           {availableSubjects.map(sub => (
                               ...
                               >
                                  <div className="flex items-center gap-3 text-left">
                                      <BookOpen className={`w-4 h-4 ${formData.subjects.includes(sub) ? 'text-amber-500' : 'text-slate-300'}`} />
                                      {sub}
                                  </div>
                                  {formData.subjects.includes(sub) && <CheckCircle2 className="w-4 h-4" />}
                               </button>
                           ))}
                        </div>
                    </div>
                ...
                <div className="flex gap-6 shrink-0 pt-6">
                    <button onClick={() => setIsModalOpen(false)} className="flex-1 py-5 text-slate-400 font-black text-sm uppercase tracking-widest hover:bg-slate-50 rounded-2xl transition-all">Discard Changes</button>
                    <button 
                    ...
                    >
                        <ShieldCheck className="w-5 h-5" /> {modalType === 'add' ? 'Authorize Mapping' : 'Commit Reconfiguration'}
                    </button>
                </div>
             </div>
          </div>
           ...
           <div className="bg-white rounded-[3rem] w-full max-w-md shadow-2xl animate-in fade-in zoom-in duration-300 overflow-hidden">
              <div className="p-10 text-center space-y-8">
                ...
                <div className="space-y-3">
                    <h2 className="text-3xl font-black text-slate-800 tracking-tight">Purge Mapping?</h2>
                    <p className="text-sm font-bold text-slate-400 px-6 leading-relaxed">
                        This will unbind all sections and subjects from <span className="text-slate-800 font-black">{selectedMapping?.className}</span>.
                    </p>
                </div>
                
                <div className="flex flex-col gap-4">
                    <button onClick={handleDelete} className="w-full py-5 bg-rose-500 text-white font-black rounded-2xl shadow-xl shadow-rose-200 hover:bg-rose-600 active:scale-95 transition-all uppercase tracking-widest text-xs">Purge Mapping</button>
