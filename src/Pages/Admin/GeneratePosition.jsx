...
const GeneratePosition = () => {
  ...
  const meritList = [
    { rank: 1, name: "মোহাম্মদ রহমান", roll: "01", gpa: "5.00", totalMarks: "895", status: "passed" },
    { rank: 2, name: "আয়েশা খাতুন", roll: "02", gpa: "4.95", totalMarks: "882", status: "passed" },
    { rank: 3, name: "আব্দুল্লাহ মামুন", roll: "05", gpa: "4.88", totalMarks: "870", status: "passed" },
    { rank: 4, name: "ফাতিমা জোহরা", roll: "04", gpa: "4.50", totalMarks: "810", status: "passed" },
  ];
    ...
    <div className="space-y-8 animate-in fade-in duration-500 p-4 md:p-8">
      ...
      <div className="bg-white rounded-[2.5rem] border-2 border-slate-200 p-8 flex flex-col md:flex-row justify-between items-center gap-6 shadow-sm">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-black text-slate-800">Generate Merit List</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
         ...
         <div className="bg-white p-8 rounded-[2.5rem] border-2 border-slate-200 shadow-sm flex flex-col items-center text-center space-y-4">
            ...
            <div>
               <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Mark Entries Finished</p>
         ...
         <div className="bg-white p-8 rounded-[2.5rem] border-2 border-slate-200 shadow-sm flex flex-col items-center text-center space-y-4">
            ...
            <div>
               <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Avg. Class Performance</p>
               <p className="text-2xl font-black text-slate-800">82.4% (GPA 4.2)</p>
            </div>
         </div>
      </div>
          ...
          <table className="w-full">
            <thead className="bg-slate-50/50">
              <tr>
                ...
                <th className="px-8 py-5 text-center text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Marks</th>
                <th className="px-8 py-5 text-center text-[10px] font-black text-slate-400 uppercase tracking-widest">GPA</th>
                <th className="px-8 py-5 text-center text-[10px] font-black text-slate-400 uppercase tracking-widest">Outcome</th>
              </tr>
            </thead>
            <tbody className="divide-y-2 divide-slate-50">
              {meritList.map((st) => (
                <tr key={st.rank} className="group hover:bg-blue-50/30 transition-colors">
                  <td className="px-8 py-6">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-sm border-2 ${
                       st.rank === 1 ? 'bg-amber-500 border-amber-600 text-white shadow-lg' :
                       st.rank === 2 ? 'bg-slate-300 border-slate-400 text-slate-700 shadow-md' :
                       st.rank === 3 ? 'bg-amber-100 border-amber-200 text-amber-700' : 'bg-slate-50 border-slate-100 text-slate-400'
                    }`}>
                       {st.rank}
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <div className="flex flex-col">
                       ...
                       <span className="text-[10px] font-black text-blue-500 uppercase tracking-widest mt-0.5">Roll: {st.roll}</span>
                    </div>
                  </td>
                  <td className="px-8 py-6 text-center font-black text-slate-700">{st.totalMarks}</td>
                  ...
                  <td className="px-8 py-6 text-center">
                    <span className="text-[10px] font-black uppercase tracking-widest text-emerald-500">{st.status}</span>
                  </td>
                </tr>
              ))}
            </tbody>
