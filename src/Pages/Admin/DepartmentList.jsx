    ...
    <div className="p-3 sm:p-4 md:p-6 lg:p-8 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[2.5rem] border-2 border-slate-100 shadow-sm relative overflow-hidden">
        ...
        <div className="flex items-center gap-5 relative z-10">
          <div className="w-16 h-16 bg-blue-50 rounded-2xl flex items-center justify-center shadow-inner">
            <Building2 className="w-8 h-8 text-[#00bd7f]" />
          </div>
          <div>
            ...
            <p className="text-slate-500 text-sm font-bold uppercase tracking-widest mt-1">
              Organize your institutional sectors
            </p>
          </div>
        </div>
        <button
          onClick={() => setShowAddModal(true)}
          className="w-full sm:w-auto flex items-center justify-center gap-2 px-8 py-3.5 text-sm font-black bg-[#00bd7f] text-white rounded-2xl shadow-xl shadow-emerald-500/30 hover:shadow-2xl hover:shadow-emerald-500/40 hover:scale-[1.02] active:scale-95 transition-all z-10 relative cursor-pointer"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredDepts.map((dept) => (
          <div
            key={dept.id}
            className="bg-white rounded-[2rem] border-2 border-slate-50 p-6 shadow-sm hover:border-blue-200 hover:shadow-xl transition-all group relative overflow-hidden"
          >
            <div
              className={`absolute top-0 right-0 w-2 h-full ${dept.color}`}
            />
            <div className="flex justify-between items-start mb-6">
              <div
                className={`w-14 h-14 ${dept.color} bg-opacity-10 rounded-2xl flex items-center justify-center`}
              >
                <Building2
          ...
          >
            <div className="p-8 border-b-2 border-slate-50 flex items-center justify-between bg-slate-50/50">
              ...
              <button
                ...
                className="p-3 bg-white border-2 border-slate-100 text-slate-400 hover:text-rose-500 rounded-xl transition-all cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            <form
              onSubmit={(e) => {
                e.preventDefault();
                handleSave(e.target.deptName.value);
              }}
              className="p-8 space-y-6"
            >
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                  <Building2 className="w-3.5 h-3.5 text-[#00bd7f]" />{" "}
                  Department Name
                </label>
                <input
                  name="deptName"
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 py-4 bg-slate-100 text-slate-600 font-black rounded-2xl  shadow-emerald-500/30 transition-all uppercase text-xs tracking-widest"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="flex-1 py-4 bg-[#00bd7f] text-white font-black rounded-2xl shadow-xl shadow-blue-200 hover:scale-[1.02] transition-all uppercase text-xs tracking-widest"
                >
                  Save
                </button>
              </div>
        ...
        >
          ...
          >
            ...
            <p className="text-sm font-bold text-slate-500 mb-8 leading-relaxed">
              Deleting <span className="text-rose-600">{deleteDept.name}</span>{" "}
              will unassign its members. This Cannot be undone.
            </p>
            <div className="flex gap-4">
              <button
                onClick={() => setDeleteDept(null)}
