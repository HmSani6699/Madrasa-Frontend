...
const ExamSetup = () => {
  ...
  const [setups, setSetups] = useState([
    { id: 1, term: "First Term 2026", class: "Class 5", subject: "Arabic Grammar", marks: 100, distribution: "General Written" },
    { id: 2, term: "First Term 2026", class: "Class 6", subject: "Mathematics", marks: 100, distribution: "MCQ Based" },
    { id: 3, term: "Mid Term 2026", class: "Class 5", subject: "Fiqh", marks: 100, distribution: "General Written" },
  ]);
    ...
    <div className="space-y-8 animate-in fade-in duration-500 p-4 md:p-8">
      ...
      <div className="bg-white rounded-[2.5rem] border-2 border-slate-200 p-8 flex flex-col md:flex-row justify-between items-center gap-6 shadow-sm">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-black text-slate-800">Exam Setup</h1>
            <p className="text-slate-500 font-bold mt-1">Map subjects to examination terms and mark schemas</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {setups.map((setup) => (
          <div key={setup.id} className="group bg-white rounded-[2.5rem] border-2 border-slate-100 shadow-sm hover:shadow-xl hover:border-emerald-500/20 transition-all duration-300 flex flex-col">
            <div className="p-8 space-y-6">
              ...
              <div className="space-y-3 pt-4 border-t-2 border-slate-50 border-dashed">
                 <div className="flex justify-between items-center text-xs font-bold text-slate-500">
                    <span className="opacity-60 uppercase tracking-widest text-[9px] font-black">Marking Schema</span>
                    <span className="text-emerald-600 font-black">{setup.distribution}</span>
                 </div>
                 <div className="flex justify-between items-center text-xs font-bold text-slate-500">
                    <span className="opacity-60 uppercase tracking-widest text-[9px] font-black">Maximum Marks</span>
                    <span className="text-slate-800 font-black">{setup.marks}</span>
                 </div>
              </div>
            </div>
        ...
        >
          ...
          <p className="text-lg font-black text-emerald-400 group-hover:text-emerald-600">Add Subject Mapping</p>
        </button>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
           <div className="bg-white rounded-[3rem] w-full max-w-xl shadow-2xl animate-in zoom-in duration-300 overflow-hidden">
              <div className="p-10 border-b-2 border-slate-50 flex items-center justify-between bg-emerald-50/30">
                 <h2 className="text-2xl font-black text-slate-800 tracking-tight">Exam Mapping Setup</h2>
              ...
              <div className="p-8 space-y-6">
                 <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2 col-span-2 sm:col-span-1">
                        ...
                        <select className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 text-sm font-bold focus:border-emerald-500 outline-none appearance-none">
                    ...
                    <div className="space-y-2 col-span-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Marking Schema</label>
                        <select className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 text-sm font-bold focus:border-emerald-500 outline-none appearance-none">
                            <option>General Written</option>
                            <option>MCQ Based</option>
                        </select>
                    </div>
                 </div>
                 <div className="pt-6 flex gap-4">
                    <button onClick={() => setIsModalOpen(false)} className="flex-1 py-4 text-slate-500 font-black">Discard</button>
                    <button onClick={() => setIsModalOpen(false)} className="flex-1 py-4 bg-emerald-600 text-white font-black rounded-2xl shadow-xl shadow-emerald-200">Save Setup</button>
                 </div>
              </div>
           </div>
        </div>
