  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 p-4 md:p-8">
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-black text-slate-800">
              Assign Exam Timetable
            </h1>
            {/* <p className="text-slate-500 font-bold mt-1">
              Create new examination schedules for student classes
            </p> */}
          </div>
        </div>

        <Link to={"/admin/exam/schedule/list"}>
          {" "}
      ...
      <div className="bg-white rounded-[2.5rem] border-2 border-slate-200 shadow-sm overflow-hidden">
        <div className="p-8 border-b-2 border-slate-50 bg-slate-50/30 gap-5 grid grid-cols-1 lg:grid-cols-2">
          <SelectInputField
            title={"Exam Name"}
            options={[{ value: "First Semister" }]}
          />
          <SelectInputField title={"Class"} options={[{ value: "Class 5" }]} />
        ...
        <div className="p-8 space-y-6">
          {scheduleItems.map((item, index) => (
            <div
              key={item.id}
              className="grid grid-cols-1 md:grid-cols-6 lg:grid-cols-12 gap-6 items-end pb-8 border-b border-slate-100 last:border-0 last:pb-0"
            >
              ...
              <div className="md:col-span-1 lg:col-span-2 space-y-2">
                ...
                <input
                  type="date"
                  className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 text-xs font-bold outline-none focus:border-[#00bd7f]"
                />
              </div>
              <div className="md:col-span-1 lg:col-span-2 space-y-2">
                <label className="text-[9px] font-black text-slate-400 uppercase flex items-center gap-2">
          ...
          <div className="pt-8 flex flex-col md:flex-row justify-between items-center gap-6">
            ...
            <div className="flex gap-4 w-full md:w-auto">
              <button className="flex-1 md:flex-none px-12 py-4 bg-slate-100 text-slate-500 rounded-2xl font-black text-sm uppercase">
                Reset
              </button>
