...
const SMS = () => {
  ...
  // Mock Data
  const stats = [
    { label: "SMS Sent", value: "1,284", icon: Send, color: "text-blue-600", bg: "bg-blue-100" },
    { label: "Available Credits", value: "8,716", icon: Hash, color: "text-emerald-600", bg: "bg-emerald-100" },
    { label: "Delivery Rate", value: "98.5%", icon: ShieldCheck, color: "text-purple-600", bg: "bg-purple-100" },
    { label: "Today's Volume", value: "142", icon: Zap, color: "text-amber-600", bg: "bg-amber-100" },
  ];

  const smsHistory = [
    { id: 1, recipient: "Abdur Rahman (STU001)", type: "Individual", message: "Guardian Meeting tomorrow at 10 AM.", status: "delivered", time: "10:30 AM" },
    { id: 2, recipient: "Class 5 - All Students", type: "Bulk", message: "Reminder: Mid-term exams start from Monday.", status: "delivered", time: "Yesterday" },
    { id: 3, recipient: "Hafiz Ahmed (Teacher)", type: "Individual", message: "Please submit the attendance report.", status: "failed", time: "Jan 12" },
    { id: 4, recipient: "Section B - Guardians", type: "Bulk", message: "Winter vacation starts from 15th January.", status: "delivered", time: "Jan 10" },
  ];
      ...
      <div className="max-w-[1600px] mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
          <div>
            ...
            <p className="text-slate-500 font-bold mt-1 uppercase tracking-widest text-[10px]">Send and manage institutional communications</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((stat, idx) => (
            <div key={idx} className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm flex items-center gap-5 group hover:border-indigo-200 transition-all">
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden flex flex-col h-full">
              <div className="flex border-b border-slate-100 bg-slate-50/50">
                <button 
                  ...
                  className={`flex-1 py-6 font-black text-sm tracking-widest uppercase flex items-center justify-center gap-2 transition-all ${
                    activeTab === "send" ? "bg-white text-indigo-600 border-b-2 border-indigo-600" : "text-slate-400 hover:text-slate-600"
                  }`}
                >
                ...
                <button 
                  ...
                  className={`flex-1 py-6 font-black text-sm tracking-widest uppercase flex items-center justify-center gap-2 transition-all ${
                    activeTab === "bulk" ? "bg-white text-indigo-600 border-b-2 border-indigo-600" : "text-slate-400 hover:text-slate-600"
                  }`}
                >
              ...
              <div className="p-8 space-y-8 flex-1">
                ...
                <div className="space-y-4">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Recipient Category</label>
                  <div className="grid grid-cols-3 gap-4">
                    {[
                      ...
                      >
                        <type.icon className={`w-6 h-6 ${recipientType === type.id ? 'text-indigo-600' : 'text-slate-300 group-hover:text-indigo-400'}`} />
                ...
                <div className="space-y-4">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">
                    {activeTab === "send" ? "Search Recipient" : "Select Targets"}
                  </label>
                  <div className="relative">
                    ...
                    <input 
                      ...
                      placeholder={activeTab === "send" ? `Search ${recipientType} by name or ID...` : `Select classes, sections or departments...`}
                    />
                  </div>
                </div>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col h-full max-h-[700px]">
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-4">
                {smsHistory.map((log) => (
                  <div key={log.id} className="p-5 bg-slate-50/50 border border-slate-100 rounded-3xl hover:bg-white hover:border-indigo-100 hover:shadow-lg hover:shadow-indigo-50 transition-all group cursor-pointer relative overflow-hidden">
                    <div className={`absolute top-0 left-0 w-1 h-full ${log.status === 'delivered' ? 'bg-emerald-500' : 'bg-rose-500'}`}></div>
                    <div className="flex justify-between items-start mb-2">
                      <span className="text-[10px] font-black text-slate-400 uppercase tracking-tighter shrink-0">{log.type}</span>
                      <span className="text-[9px] font-bold text-slate-400 uppercase">{log.time}</span>
                    </div>
                    <h4 className="text-sm font-black text-slate-800 truncate mb-1 pr-4">{log.recipient}</h4>
                    <p className="text-xs text-slate-500 font-medium line-clamp-2 leading-relaxed mb-3">{log.message}</p>
                    <div className="flex items-center justify-between pt-3 border-t border-slate-100">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className={`text-[10px] font-black uppercase tracking-tight ${log.status === 'delivered' ? 'text-emerald-600' : 'text-rose-600'}`}>
                          {log.status}
                        </span>
                      </div>
               ...
               <div className="relative z-10">
                 ...
                 <ul className="space-y-4">
                   {[
                     ...
                     <li key={i} className="flex gap-3 text-sm font-bold text-indigo-100 items-start">
      ...
      {showSuccessToast && (
        <div className="fixed bottom-12 left-1/2 -translate-x-1/2 z-[200] animate-in slide-in-from-bottom-20 duration-500">
          <div className="bg-slate-900 border border-slate-800 backdrop-blur-xl py-5 px-8 rounded-[2rem] shadow-2xl flex items-center gap-4">
            ...
            <div>
              <p className="text-white font-black tracking-tight">SMS Sent Successfully!</p>
              <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">Message has been queued for delivery.</p>
            </div>
          </div>
        </div>
      )}
