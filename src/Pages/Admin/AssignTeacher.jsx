    ...
    <div className="space-y-8 animate-in fade-in duration-500 p-4 md:p-8">
      ...
      <div className="bg-white rounded-[2.5rem] border-2 border-slate-200 p-8 flex flex-col md:flex-row justify-between items-center gap-6 shadow-sm">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-black text-slate-800">Assign Class Teacher</h1>
            <p className="text-slate-500 font-bold mt-1">Map teachers to lead specific classes/sections</p>
          </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-6">
        <div className="flex-1 bg-white rounded-3xl border-2 border-slate-200 p-2 flex items-center shadow-sm">
            <div className="flex-1 px-6">
               <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Live Database Search</label>
        ...
        <div className="bg-gradient-to-br from-indigo-600 to-blue-700 rounded-3xl p-6 text-white shadow-xl shadow-blue-100 min-w-[240px] flex flex-col justify-center">
           <p className="text-[10px] font-black uppercase opacity-60 tracking-widest">Global Capacity</p>
           <div className="flex items-center justify-between mt-2">
               <p className="text-3xl font-black tracking-tighter">{assignments.length}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
        {filteredAssignments.map((as) => (
          <div key={as.id} className="group bg-white rounded-[2.5rem] border-2 border-slate-100 shadow-sm hover:shadow-xl hover:border-blue-500/20 transition-all duration-300 overflow-hidden flex flex-col relative">
            <div className="p-8 space-y-6">
              <div className="flex items-center justify-between">
                ...
                <div className="relative" ref={activeMenuId === as.id ? menuRef : null}>
                    <button 
                        onClick={() => setActiveMenuId(activeMenuId === as.id ? null : as.id)}
              ...
              <div className="space-y-4 pt-2">
                <div className="flex items-center gap-4 text-xs font-bold text-slate-500 group/link">
                  <div className="w-10 h-10 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-400 group-hover/link:bg-blue-50 group-hover/link:text-blue-500 transition-colors"><Mail className="w-4 h-4" /></div>
                  <div className="flex flex-col">
                    <span className="text-[10px] uppercase font-black tracking-widest text-slate-300">Email Contact</span>
                    <span className="group-hover/link:text-slate-800 transition-colors">{as.email || "assign@mms.edu"}</span>
                  </div>
                </div>
                <div className="flex items-center gap-4 text-xs font-bold text-slate-500 group/link">
                  <div className="w-10 h-10 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-400 group-hover/link:bg-emerald-50 group-hover/link:text-emerald-500 transition-colors"><Phone className="w-4 h-4" /></div>
                  <div className="flex flex-col">
                    <span className="text-[10px] uppercase font-black tracking-widest text-slate-300">Secure Phone</span>
                    <span className="group-hover/link:text-slate-800 transition-colors">{as.phone || "+880 1XXX-XXXXXX"}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        >
          ...
          <p className="text-xl font-black text-blue-300 group-hover:text-blue-600 transition-colors">Assign New Lead</p>
          <p className="text-xs font-bold text-blue-200 group-hover:text-blue-400 mt-2 uppercase tracking-widest">Map teacher to class</p>
        </button>
      </div>
          ...
          <div className="bg-white rounded-[2.5rem] w-full max-w-xl shadow-2xl animate-in zoom-in duration-300 overflow-hidden">
             <div className="p-8 border-b-2 border-slate-50 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-blue-50 text-blue-600 rounded-2xl flex items-center justify-center border-2 border-blue-100">
             ...
             <div className="p-8 space-y-8">
                <div className="grid grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Academic Section</label>
                        <select 
                    ...
                    <div className="space-y-2 col-span-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Designated Teacher</label>
                        <select 
                    ...
                    <div className="space-y-2 col-span-2 sm:col-span-1">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Professional Experience</label>
                        <input 
                            value={formData.experience}
                            onChange={(e) => setFormData({ ...formData, experience: e.target.value })}
                            placeholder="e.g. 10 Years"
                            className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 text-sm font-bold focus:border-blue-500 outline-none"
                        />
                    </div>
                    <div className="space-y-2 col-span-2 sm:col-span-1">
                ...
                <div className="p-4 bg-blue-50 rounded-2xl border-2 border-blue-100 flex gap-3">
                   ...
                   <p className="text-[11px] font-bold text-blue-700 leading-relaxed">
                      Assigning a Class Teacher gives them oversight of attendance, marks, and student discipline for this specific section.
                   </p>
                </div>

                <div className="flex gap-4 pt-4">
                    <button onClick={() => setIsModalOpen(false)} className="flex-1 py-4 text-slate-500 font-black rounded-2xl hover:bg-slate-50 transition-all">Discard Changes</button>
                    <button onClick={handleAction} className="flex-1 py-4 bg-blue-600 text-white font-black rounded-2xl shadow-xl shadow-blue-200 hover:bg-blue-700 transition-all flex items-center justify-center gap-2">
                        <CheckCircle2 className="w-5 h-5" /> {modalType === 'add' ? 'Confirm Lead' : 'Save Updates'}
                    </button>
                </div>
             </div>
          </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
           <div className="bg-white rounded-[2.5rem] w-full max-w-sm shadow-2xl animate-in fade-in zoom-in duration-300 overflow-hidden">
              <div className="p-8 text-center space-y-6">
                ...
                <div className="space-y-2">
                    <h2 className="text-2xl font-black text-slate-800 tracking-tight">Unassign Lead?</h2>
                    <p className="text-sm font-bold text-slate-400">This will remove management privileges for</p>
                </div>
                
                <div className="p-4 bg-slate-50 rounded-2xl border-2 border-slate-100">
                    <p className="text-lg font-black text-slate-800">{selectedAssignment?.teacherName}</p>
                    <p className="text-[10px] font-black text-blue-500 uppercase tracking-widest mt-1">{selectedAssignment?.className} • {selectedAssignment?.section}</p>
                </div>

                <div className="flex gap-4 pt-2">
                    <button onClick={() => setIsDeleteModalOpen(false)} className="flex-1 py-4 font-black text-slate-400 hover:bg-slate-50 rounded-2xl transition-all">Back</button>
                    <button onClick={handleDelete} className="flex-1 py-4 bg-rose-500 text-white font-black rounded-2xl shadow-xl shadow-rose-200 hover:bg-rose-600 transition-all">Yes, Remove</button>
                </div>
              </div>
           </div>
        </div>
